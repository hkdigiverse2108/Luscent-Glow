...
const AdminSettings = () => {
  ...
  return (
    <div className="space-y-2 pb-4">
      ...
      <div className="grid grid-cols-1 gap-4 max-w-4xl">
        {settingsGroups.map((group, i) => (
          <motion.section 
            ...
            className={`relative overflow-hidden backdrop-blur-3xl border rounded-3xl p-4 transition-all duration-700 ${
              isDark ? "bg-charcoal/40 border-white/12 shadow-black/50" : "bg-white border-charcoal/12 shadow-charcoal/15"
            }`}
          >
            {/* Group Header */}
            <div className={`flex flex-col md:flex-row md:items-center gap-4 mb-2 border-b pb-2 transition-colors duration-700 ${
              isDark ? 'border-white/12' : 'border-charcoal/12'
            }`}>
              <div className="w-12 h-12 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-500 shadow-2xl">
                <group.icon size={24} />
              </div>
              <div className="space-y-2">
                <h3 className={`text-2xl font-extrabold uppercase tracking-tight transition-colors duration-700 ${
                  isDark ? "text-white" : "text-charcoal"
                }`}>{group.title}</h3>
                <p className={`text-sm font-extrabold transition-colors duration-700 ${
              ...
              {group.settings.map((setting) => (
                <div key={setting.label} className="flex flex-col md:flex-row md:items-center justify-between gap-8 group/item">
                  <div className="space-y-2">
                    ...
                    <p className={`text-lg font-extrabold uppercase tracking-widest transition-colors duration-700 ${
      ...
      <div className={`mt-4 p-4 border rounded-3xl flex items-center justify-between transition-all duration-700 ${
        isDark ? "bg-white/5 border-white/12 shadow-black/50" : "bg-charcoal/5 border-charcoal/12 shadow-charcoal/10"
      }`}>
           <div className="flex items-center gap-4">
             <div className="w-12 h-12 rounded-full bg-gold/10 flex items-center justify-center text-gold shadow-2xl">
               <Sparkles size={24} />
             </div>
             <div>
